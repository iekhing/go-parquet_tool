...
func CreateSchemaTree(schemas []*parquet.SchemaElement) *SchemaTree {
	...
	for len(stack) > 0 {
		...
		fmt.Printf("node name %s stack length %d num of children %d \n", node.SE.GetName(), len(stack), node.SE.GetNumChildren())
		...
		if lc < numChildren {
			...
			fmt.Printf("new node name %s stack length %d num of children %d \n", newNode.SE.GetName(), len(stack), newNode.SE.GetNumChildren())
